...
| `docs/product/` | Product/GTM strategy layer (folded in 2026-07-16 from an off-site working session): opportunity memo, product spec, competitive landscape, gap analysis, LLM eval framework + 50-case scaffold suite, veracium intelligence-layer assessment. **Proposals, not adopted policy** — the operating principles below still govern; see `docs/product/README.md` for status and the posture-change caveat. |
...
747 backend unit tests + 148 frontend tests + a Playwright/axe e2e suite (3 specs) passing; Postgres/Bedrock/Gmail/browser/schema suites deselect by default. Ruff, mypy strict, and ruff-format clean across 171 source files; frontend `vite build` clean.

**C8 (operability & polish) — complete (cosmetic polish continuous); the planned canvas cuts C1–C8 are done.** **C8.1 batch run**: `POST /api/workflows/{id}/run-batch` (Admin/Operator) fires one instance per row of a JSON array of payloads, bounded-concurrent (`asyncio.Semaphore`, cap 5; ≤100 rows), isolating per-row failures and returning `{submitted, succeeded, failed, results}`. The Run dialog gained a **Batch** tab (paste JSON / upload CSV-or-JSON; `lib/batch.ts` parses both) showing a per-row results list with links to each run. **C8.2 a11y baseline + harness**: a Playwright + `@axe-core/playwright` E2E suite (`frontend/e2e/`, own CI `e2e` job) drives create→edit→save (the "Saved ✓" path)→delete in a real browser and runs axe scans on home/templates/canvas — the first run caught a `mode-pill` contrast bug, since fixed (it's the verification surface jsdom can't be). Loading skeletons / responsive breakpoint remain as continuous polish. Next: the deferred epics **E1–E5** (sub-workflows, collab, AI-layout-B, delivery surfaces, version history), each trigger-gated.
...
**Third validation workload: DMARC report ingest** (`examples/dmarc_ingest/`) — the first fully **deterministic** production workflow (zero LLM tokens). Pulled in the email connector's attachment-read slice: `EmailAttachment` metadata on `EmailMessage`, `GmailConnector.download_attachment` (base64url + re-pad), `poll_inbox(query=…)` raw Gmail search passthrough, and `GmailPollTrigger(query=, download_dir=)` which spools attachments to `<dir>/<message_id>/<filename>` (hostile filenames flattened) and puts `attachment_paths` on the payload — deterministic steps can't reach connectors, so the trigger delivers files. New stock functions: `extract_archive` (zip + gzip via `world.fs`, zip-slip-safe member flattening, suffix filter, collision-prefixed names; skips non-archives non-fatally) and `copy_files` (generic deliver-to-directory: basename-flattened, suffix filter, idempotent overwrite) — the workflow is trigger + two reusable steps (unzip → staging, deliver → destination), per the steps-are-reusable-catalog-capabilities principle. Delivery target is the dmarc-viewer app's watched dir (`/home/ubuntu/Dev/dmarc-viewer/xml-files` — it auto-loads any `.xml` written there). Backfill via `backend/tools/fetch_dmarc.py` (historical Gmail scan → spool → `--fire` POSTs run-batch per message); ongoing mail rides the normal email trigger. See `docs/MANUAL_TESTING.md` §4e. Multi-account: the orchestrator seeds each `gmail_poll` trigger's *own* account from `.secrets/gmail/<account>/` (`seed_gmail_env_from_disk`, dev/EnvSecretStore path) — previously only the single tools account (`WORKFLOW_PLATFORM_GMAIL_ACCOUNT`) was seeded, so a trigger polling a second mailbox self-disabled despite creds on disk; `run-local-be.sh` pre-flight now lists every credentialed account. **Trigger taxonomy:** the semantic trigger type is **`email`** with `provider` in config (only `gmail` today; unknown providers warn + skip) — trigger types name the *event*, providers/mechanisms are config, matching `filesystem`/`schedule`/`webhook`. `gmail_poll` remains an accepted legacy alias (same precedent as `file_watch`→`filesystem`); examples + catalog migrated to `email`.

**Learned per-entity memory — veracium adopted, write-only slice live (2026-07-13).** The email-triage live validation (~1 day live + 107-message historical batch on the personal mailbox) fired the adoption trigger logged in `docs/SEMANTICS.md`; a design-reviewer pass said adopt-with-conditions, all implemented: engine-written observations only (no agent memory tool — COALA N1 stands), veracium's LLM calls routed through `BedrockClient` (metered, audited, replayable), dry-run writes go to an ephemeral scratch DB. New `workflow_platform.memory.learned.LearnedMemoryService` (sync veracium behind `to_thread`, loop-hopping `Complete` adapter, wiki recompile disabled → one Haiku distill call per observation) + opt-in `learned_memory:` block on `WorkflowDefinition` (templated observations, `third_party`/`system`/`user` authorship; email content is always `third_party` → claims quarantined). Every write lands a `memory_observed` audit entry (content hash, facts/quarantined counts, tokens, cost); spend lands in instance totals. Store: `WORKFLOW_PLATFORM_LEARNED_MEMORY_DB` (default `.memory/learned.db`); dep pinned `veracium==0.2.1` (PyPI; 0.1.6→0.2.1 upgrade + `derived_from` mixed-provenance + full store rebuild landed 2026-07-17 after the laundering fix this project reported shipped upstream). First consumer: `examples/email_triage_live/`. **Recall injection (G10, slice 2) is live (2026-07-17):** opt-in `recall:` block (`query_from` + `token_budget`); the engine normalizes the entity (case/plus-addressing), calls `recall_context` (zero LLM cost — subgraph render, wiki off), and injects veracium's block **verbatim** below the rubric with the never-assert fence intact (both security acceptance criteria test-pinned); `memory_recalled`/`memory_recall_failed` audit entries; `recall` field on agent step output; dry runs snapshot-copy the store (read-realistic, write-discarded). Outcome-event emission awaits veracium 0.3.x (PR #9, consumer-validated 26/26).

**Users + organizations skeleton (2026-07-19).** Built deliberately ahead of need so user-adjacent features stop hardcoding identities: `users` + `organizations` tables (Alembic `0003`; single `default` org seeded in-migration), JIT provisioning from the IdP identity (`(iss, sub)` key, TTL-throttled last-seen upsert in `AuthMiddleware`), `GET /api/me`, and ownership attribution (`org_id` on definitions + instances from birth; `owner_user_id` set by the API create/import/scaffold paths). D4 unchanged: the IdP owns authn + roles, nothing credential-shaped is persisted, audit `actor_id` stays the raw sub. Enforcement/multi-tenancy deliberately deferred; veracium namespace migration to `user:<id>` is the decided follow-up.
