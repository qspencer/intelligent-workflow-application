...
695 backend unit tests + 146 frontend tests passing; Postgres/Bedrock/Gmail/browser/schema suites deselect by default. Ruff, mypy strict, and ruff-format clean across 167 source files; frontend `vite build` clean.

**C8 (operability & polish) — in progress.** **C8.1 batch run** shipped: `POST /api/workflows/{id}/run-batch` (Admin/Operator) fires one instance per row of a JSON array of payloads, bounded-concurrent (`asyncio.Semaphore`, cap 5; ≤100 rows), isolating per-row failures and returning `{submitted, succeeded, failed, results}`. The Run dialog gained a **Batch** tab (paste JSON / upload CSV-or-JSON; `lib/batch.ts` parses both) showing a per-row results list with links to each run. Remaining: C8.2 polish / a11y / responsive.
