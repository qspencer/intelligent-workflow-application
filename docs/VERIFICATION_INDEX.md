...
| F3 tool secrets (incl. model-ECHOED output_text), RAW trigger payloads, and recalled correspondent history projected across every read surface | `api/redaction.py::redact_tool_data` — redacts `tool_calls`, withholds `output_text` when the step used a tool, projects `trigger_payload`/`trigger` to routing-only (`safe_trigger_payload`), and withholds `recall`; applied to both audit endpoints, instance detail (dump+steps+context), explain, WS delivery; the LIST endpoint returns a `_instance_summary` that OMITS context + trigger_payload entirely (external review round 4) | `test_audit_redaction.py::test_below_admin_cannot_recover_tool_secrets_anywhere` (model echoes both secrets into output_text; a raw trigger body is set; Viewer+User recover no tool/output/trigger secret from all 5 HTTP surfaces incl. the list; routing `message_id` survives; admin raw; admin list trace-free) **+** `test_audit_redaction.py::test_redact_projects_trigger_payload_and_recall` (trigger + recall unit) **+** `test_org_isolation.py::test_ws_redacts_tool_secrets_for_non_admin_subscriber` (WS) | **VERIFIED-CODE+TEST** for the read surfaces, now incl. the trigger-payload/recall detail-endpoint leak (internal review F4/F1/F7). NOTE: authorization is still Admin+Org-Admin roles, not a distinct raw-trace privilege, and raw access is not itself audited — storage-level separation + a distinct audited grant remain pre-external-org gates, now specified in `docs/TRACE_GOVERNANCE_PLAN.md` (F3-gate) |
