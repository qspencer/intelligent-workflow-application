...
| Gap | Disposition |
|---|---|
| **Declared safe-output is an approved DECLASSIFICATION channel across a governance boundary** (TRACE_GOVERNANCE_PLAN §1.4a). A prompt-injected — or deliberately re-prompted — agent steers *selection* among declared literals; it cannot author content, but its choices land in the operational store, which Contract B1 treats as readable by an untrusted DB/backup operator. Capacity is **computed per declaration** (Σ log₂\|domain\|, incl. list length/membership/order), NOT the ~32 bits/attempt previously claimed here — that figure assumed scalar enums and was wrong for lists (an ordered 16-member list with `max_items:16` alone carries ~64 bits). Finite per attempt, **UNBOUNDED over repeated execution**, with a hostile sender able to influence run volume. | **[NOT YET ACCEPTED — §1.4a is not build-authorized]** External design review 2026-08-04 deferred it. Named honestly as a declassification channel, not a "safe" one. Required controls now in §1.4a: approval binds the **step revision** (a prompt-only edit invalidates it — closes the codebook attack), **dual-control** authority matching §2.1, finite canonical domains (integer lattice; no float form), save-time capacity budget, revocation lifecycle, full §1.3 per-field consumer/role/retention scope. **Deferred:** an enforceable *aggregate* budget + rate boundary — trigger: first external tenant, or any raise of the per-step budget |
