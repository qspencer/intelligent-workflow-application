...
| Gap | Disposition |
|---|---|
| **Declared DECLASSIFICATION is an approved channel across a governance boundary** (TRACE_GOVERNANCE_PLAN §1.4a). A prompt-injected — or deliberately re-prompted — agent steers *selection* among declared literals; it cannot author content, but its choices land in the operational store, which Contract B1 treats as readable by an untrusted DB/backup operator. Capacity is computed per declaration over every **observable** state (accepted values + absent + redacted + status), not just accepted values — partial projection itself carries signal. Finite per attempt (`POLICY_BUDGET_BITS = 32`), **UNBOUNDED over repeated execution**. **Revocation cannot retract bytes already copied into dumps, backups, replicas, query logs, or exports — a wrongly-approved declaration is an exposure INCIDENT, not a reversible config mistake.** | **[NOT YET ACCEPTED — §1.4a is not build-authorized]** Three external design reviews; the third (2026-08-05) approved the direction and deferred on authorization lifecycle, now folded. Required controls in §1.4a: approval is a first-class object whose `approval_id`+generation binds every produced row (revoke-then-reapprove cannot resurrect rows), atomic CAS lifecycle with hard revocation of in-flight attempts, step-revision binding (a prompt-only edit invalidates it), dual-control + narrowly-scoped tenant artifact, finite canonical domains, destination-scoped projection, frozen RFC-8785/SHA-256 hashing. **External onboarding stays gated on the deferred AGGREGATE capacity + rate boundary** |
